...
export default function AdminDashboard({ onLogout }) {
  ...
  const latestDate = useMemo(
    () => submissions.reduce(
      (latest, item) => (!latest || new Date(item.submittedAt) > new Date(latest) ? item.submittedAt : latest),
      null,
    ),
    [submissions],
  );

  /** Clear the demo session, update the route state, and return home. */
  ...
  function handleSave(values) {
    updateSubmission(editing.id, values);
      ...
      <div className="admin-shell">
        ...
        <div className="stats-grid">
          ...
          <article className="stat-card success">
            <p>Departments</p>
            <strong>{new Set(submissions.map((item) => item.department)).size}</strong>
          </article>
          <article className="stat-card warning">
            <p>Latest Submission</p>
            <strong>{formatSubmissionDate(latestDate)}</strong>
          </article>
        </div>
        <SubmissionTable onDelete={handleDelete} onEdit={setEditing} submissions={submissions} />
        {editing && <EditModal onCancel={() => setEditing(null)} onSave={handleSave} submission={editing} />}
      </div>
    </section>
  );
}
